...
options:
    ...
    dryrun:
        ...
        default: false
        description:
            - Don't actually remove anything, just report what would have been deleted if dryrun was not set to true
    keep:
        required: false
        default: "0"
        description:
...
def main():
    module = AnsibleModule(
            argument_spec   = dict(
                ...
                dryrun      = dict(default='no', type='bool'),
    ...
    for d in dirs_to_delete:
        if os.path.exists(d):
            try:
                if not params['dryrun'] and (params['force'] or len(os.listdir(d)) == 0):
                    if os.path.islink(d):
                        os.remove(d)
                    else:
                        shutil.rmtree(d)
